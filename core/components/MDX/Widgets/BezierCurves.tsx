...
const Cubic = ({ scale, activeStepIndex }: CurveProps) => {
  ...
  const lerp1 = useMemo(() => lerp({ x: x0, y: y0 }, { x: x1, y: y1 }), []);
  const lerp2 = useMemo(() => lerp({ x: x1, y: y1 }, { x: x2, y: y2 }), []);
  const lerp3 = useMemo(() => lerp({ x: x2, y: y2 }, { x: x3, y: y3 }), []);
  const lerp4 = lerp(
...
const Quadratic = ({ scale, activeStepIndex }: CurveProps) => {
  const getX = (d: Point) => d.x;
  const getY = (d: Point) => d.y;
...
const Linear = ({ scale, activeStepIndex }: CurveProps) => {
  const getX = (d: Point) => d.x;
  const getY = (d: Point) => d.y;
  ...
  const data = useMemo(() => lerp({ x: x0, y: y0 }, { x: x1, y: y1 }), []);
...
const Chart = ({ width, type }: ChartProps) => {
  ...
  return (
    <Flex direction="column" alignItems="center" css={{ gap: '28px' }}>
      ...
      <Flex direction="column" alignItems="flex-start" css={{ width: '50%' }}>
        <div>
          t:{' '}
          <HighlightedValue>
...
const BezierCurves = ({ type }: BezierCurvesProps) => {
  const titlePerType = {
