...
const Chart = ({ width, editable }: ChartProps) => {
  /**
   * We use the same dimension for both X and Y axis as the chart is square
   */

  const [P1, setP1] = useState({ x: 0.16, y: 0.69 });
  const [P2, setP2] = useState({ x: 0.72, y: 0.22 });
  const [type, setType] = useState(options[0].value);

  const [activeStepIndex, setActiveStepIndex] = useState(0);
  const [isDragging, setIsDragging] = useState(false);
  const [showHandles, setShowHandles] = useState(true);
  const [showTrace, setShowTrace] = useState(!editable);
  const [traceOnChart, setTraceOnChart] = useState(editable);
  const [paused, setPaused] = useState(false);
  const [showDerivative, setShowDerivative] = useState(false);

  const dimension = editable ? width - 100 : width / 1.5;

  const data = useMemo(() => cubic(P1, P2), [P1, P2]);
  const speed = useMemo(() => cubicDerivative(P1, P2), [P1, P2]);
...
const CubicBezierVisualizer = ({ editable }: CubicBezierVisualizerProps) => {
  return (
    <Card
      css={{
